...
public class Processor {
	...
	/**
	 * Regex delimiter to be used for splitting the line read from the file into
	 * tokens
	 */
	...
	/**
	 * List of all categories present in the input file, can have duplicate
	 * values
	 */
	private static List<String> categoryList = new ArrayList<String>();

	/**
	 * Map containing all unique category and name pair present in the input
	 * file
	 */
	...
	private static String createCategoryCounts() {
		...
		for (Integer order : Categories.getCategoryOrders()) {
			...
			categoryCountBuilder.append(Collections.frequency(categoryList, Categories.getCategoryValue(order)));
			categoryCountBuilder.append(System.getProperty("line.separator"));
		}
	...
	public static String createResults() {
		return createCategoryCounts() + createCategoryValues();
	}

	/**
	 * Method to initialize the file reader, read the input file and process
	...
	/**
	 * Method processing the string token array for a certain line number from
	 * the input file, if that value has not been seen before it will be saved
	 * in the categoryValues map
	 * 
	 * @param token[]
	 *            string array
	 * @param lineNumber
	...
	private static void processTokens(String[] tokens, int lineNumber) {
		...
		if (!names.contains(value.toString())) {
			categoryValues.put(new Integer(lineNumber), new Entity(tokens[0], value.toString()));
		}
		names.add(value.toString());
	}

	/**
	 * Method to tokenize a given line using the space delimiter, this will
	 * parse out the category as the first token and its corresponding value as
	 * subsequent tokens
	 * 
	 * @param line
	 *            to be tokenized
